...
def main():
    ...
    slow_start_data = dm.pickle_load( "slow_start_data.pickle" )
    ...
    for jockey_id in tqdm( jockey_full_data.keys() ):
        ...
        for i in range( len( key_list ) - 1, -1, -1 ):
            ...
            if not result[jockey_id][key]["all"]["count"] == 0:
                ...
                result[jockey_id][key]["all"]["slow"] /= count
                ...
                result[jockey_id][key]["100"]["slow"] /= count100
            ...
            try:
                ...
                horce_id = jockey_full_data[jockey_id][key]["horce_id"]                
            except:
                continue                

            try:
                slow_data = slow_start_data[horce_id]
                day = key[0:4] + "/" + key[4:6] + "/" + key[6:8]
                slow = int( slow_data[day] )
            except:
                slow = 0

            past_data.append( { "time": time / dist, "rank": rank, "up": up, "fhb": first_horce_body, "slow": slow } )
            instance["count"] += 1
            instance["rank"] += rank
            instance["time"] += time / dist
            instance["up"] += up
            instance["fhb"] += first_horce_body
            instance["slow"] += slow
            ...
            if instance100["count"] >= 100:
                ...
                instance100["fhb"] -= past_data[p]["fhb"]
                instance100["slow"] -= past_data[p]["slow"]
            ...
            instance100["up"] += up
            instance100["fhb"] += first_horce_body
            instance100["slow"] += slow
