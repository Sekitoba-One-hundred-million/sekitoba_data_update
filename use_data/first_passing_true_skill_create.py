...
def main():
    ...
    for horce_id in horce_rating_data.keys():
        first_passing_true_skill_prod_data["horce"][horce_id] = horce_rating_data[horce_id].mu

        if horce_id in update_horce_id_list:
            horce_data.update_data( COLUM_NAME, first_passing_true_skill_prod_data["horce"][horce_id], horce_id )

    for jockey_id in jockey_rating_data.keys():
        first_passing_true_skill_prod_data["jockey"][jockey_id] = jockey_rating_data[jockey_id].mu
        ps.JockeyData().update_data( COLUM_NAME, first_passing_true_skill_prod_data["jockey"][jockey_id], jockey_id )

    for trainer_id in trainer_rating_data.keys():
        first_passing_true_skill_prod_data["trainer"][trainer_id] = trainer_rating_data[trainer_id].mu
        ps.TrainerData().update_data( COLUM_NAME, first_passing_true_skill_prod_data["trainer"][trainer_id], trainer_id )
    ...
    for race_id in update_race_id_list:
        for kind in dev_result.keys():
            ...
            first_passing_true_skill_data[kind][race_id] = {}
            for kind_id in dev_result[kind][race_id].keys():
                ...
                first_passing_true_skill_data[kind][race_id][kind_id] = dev_result[kind][race_id][kind_id]

    dm.pickle_upload( "first_passing_true_skill_data.pickle", first_passing_true_skill_data )
    dm.pickle_upload( "first_passing_true_skill_prod_data.pickle", first_passing_true_skill_prod_data )
